...
/** Immutable root-local geometry and host-local sink bounds for Baidu keyboard glass. */
final class BaiduInputMethodGlassGeometry {
    final int rootWidth;
    ...
    private BaiduInputMethodGlassGeometry(
            int rootWidth,
            int rootHeight,
            float left,
            float top,
    ...
    static BaiduInputMethodGlassGeometry capture(
            View root,
            View sinkHost,
            View target,
        ...
        try {
            ...
            float scaleX = (rootBounds.right - rootBounds.left) / Math.max(1f, target.getWidth());
            float scaleY = (rootBounds.bottom - rootBounds.top) / Math.max(1f, target.getHeight());
            float scale = Math.min(scaleX, scaleY);
            if (!finite(scale) || scale <= 0f) return null;

            return new BaiduInputMethodGlassGeometry(
    ...
    boolean sameAs(BaiduInputMethodGlassGeometry other) {
        return other != null
