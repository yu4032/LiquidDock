 ...
 *
 * <p>Launcher 4.50 derives Workspace/Dock/open-Folder ShortcutIcon and FolderIcon1x1 geometry
 * from GridConfig during onMeasure(). We scope the override to those vendor measurement
 * transactions with a ThreadLocal, so shared GridConfig instances are never mutated and
 * All Apps/search keep the vendor size.</p>
 */
final class Launcher450IconSizeHook {
    ...
    private enum MeasureDomain { WORKSPACE, DOCK, FOLDER, FOLDER_CONTENT }
    ...
    private static MeasureDomain shortcutDomain(View view) {
        ...
        if (isInsideOpenFolder(view)) return MeasureDomain.FOLDER_CONTENT;
    ...
    private static boolean isInsideOpenFolder(View view) {
        for (ViewParent parent = view.getParent(); parent != null; parent = parent.getParent()) {
            if (parent instanceof View && FOLDER_GRID_VIEW.equals(parent.getClass().getName())) {
                return true;
            }
        }
        return false;
