...
public final class PresetManager {
    private static final Map<String, Object> DEFAULT_VALUES = createNeutralValues();
    private static final Map<String, Object> MIGRATION_VALUES = createMigrationValues();
    private static final Map<String, Object> TUNED_VALUES = createTunedValues();

    private PresetManager() {}

    /** GUI default preset: switches off and numeric values zeroed. */
    public static Map<String, Object> defaultValues() {
        return DEFAULT_VALUES;
    }

    /** Safe historical preset used only by config migration. */
    public static Map<String, Object> migrationValues() {
        return MIGRATION_VALUES;
    }

    /** Full tuned preset imported from the bundled profile, including switches. */
    public static Map<String, Object> tunedValues() {
        return TUNED_VALUES;
    }

    public static void applyDefault(SharedPreferences.Editor editor) {
        writeValues(editor, DEFAULT_VALUES);
    }

    public static void applyTuned(SharedPreferences.Editor editor) {
        writeValues(editor, TUNED_VALUES);
    }
    ...
    private static Map<String, Object> createNeutralValues() {
        ...
        for (ConfigKey<?> key : ConfigSchema.all()) {
            ...
            if (key.type() == ConfigKey.Type.BOOLEAN) {
                values.put(key.name(), Boolean.FALSE);
            } else if (key.type() == ConfigKey.Type.INT) {
                values.put(key.name(), 0);
                if (key.storageMode() == ConfigKey.StorageMode.DP_TENTHS) {
                    values.put(key.name() + "_tenths", 0);
                }
            } else if (key.type() == ConfigKey.Type.STRING && key.uiDefault() != null) {
                // Strings cannot be represented by numeric zero; keep a valid enum/base value.
                values.put(key.name(), key.uiDefault());
            }
        }

        // Reset the bundled hidden profile as well, so applying the neutral preset after the tuned
        // preset does not leave third-party glass appearance behind.
        values.put("third_party_glass.systemui.lockscreen_clock.enabled", Boolean.FALSE);
        values.put("third_party_glass.systemui.lockscreen_clock.blur", 0f);
        values.put("third_party_glass.systemui.lockscreen_clock.tint_r", 0);
        values.put("third_party_glass.systemui.lockscreen_clock.tint_g", 0);
        values.put("third_party_glass.systemui.lockscreen_clock.tint_b", 0);
        values.put("third_party_glass.systemui.lockscreen_clock.tint_alpha", 0);
        return Collections.unmodifiableMap(values);
    }

    private static Map<String, Object> createTunedValues() {
    ...
    private static Map<String, Object> createMigrationValues() {
        Map<String, Object> values = new LinkedHashMap<>();
        for (ConfigKey<?> key : ConfigSchema.all()) {
