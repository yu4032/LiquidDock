...
final class LauncherGlassHomePresentationHook {
    ...
    /**
     * Launcher PREPARE is the early freeze signal. Only a successful endpoint rollover may release
     * the source barrier; failed recovery remains fail-closed until a later PREPARE starts a new
     * serial.
     */
    private static void hookUnlockState(ClassLoader classLoader) {
        try {
            ...
            MainHook.log(TAG + " unlock freeze installed PREPARE; release=successful endpoint rollover");
        } catch (Throwable error) {
    ...
    private static void applyUnlockDecision(
        ...
        LauncherGlassSessionRegistry.prepareUnlockCaptureReturn(success -> {
            ...
            if (!finished.releaseBarrier) {
                if (finished.failedClosed) {
                    MainHook.log(TAG + " unlock endpoint rollover failed; entering FAILED_CLOSED"
                            + " serial=" + serial);
                    LauncherGlassSessionRegistry.failClosedUnlockCapture();
                }
                return;
            }
