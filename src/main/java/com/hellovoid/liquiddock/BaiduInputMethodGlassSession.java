...
/** Continuous zero-copy PassBlur -> Prismal pipeline for one supported Baidu keyboard root. */
final class BaiduInputMethodGlassSession implements RootPassBlurBackend.Consumer {
    interface Listener {
        void onPresented();
    ...
    private volatile BaiduInputMethodGlassGeometry geometry;
    ...
    BaiduInputMethodGlassSession(
        ...
        sourceBackend = new RootPassBlurBackend(
                root,
                PassBlurBindRequest.baiduInputMethod(root),
                scalePercent,
                renderFps,
                this,
                "LiquidDock-BaiduInputMethod-EGL");
    }
    ...
    void updateGeometry(BaiduInputMethodGlassGeometry next) {
        if (shuttingDown || next == null) return;
        BaiduInputMethodGlassGeometry old = geometry;
    ...
    /** Called only after TextureView consumed a swapped buffer. */
    ...
    private void renderCurrent() {
        BaiduInputMethodGlassGeometry currentGeometry = geometry;
