        ...
        if (SecurityCenterProcessPolicy.PACKAGE.equals(packageName)) {
            ...
            try {
                ...
                if (!SecurityCenterPassBlurContinuousAuthority.install()) {
                    Api101Bridge.log("[DC][SecurityCenterGlass] continuous PassBlur authority unavailable; fail closed");
                    return;
                }
                if (!SecurityCenterVendorMaterialState.install()) {
                    Api101Bridge.log("[DC][SecurityCenterGlass] vendor material interception unavailable; fail closed");
                    return;
                }
                if (!SecurityCenterSourceAuthorityHook.install(classLoader)) {
                    Api101Bridge.log("[DC][SecurityCenterGlass] activity authority unavailable; continuing with vendor-hosted lifecycle");
                }
