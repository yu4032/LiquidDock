...
/** Requires standard AppWidgets to mirror Launcher 4.50's RemoteViews background owner lookup. */
public class WidgetRemoteViewsRootBackgroundContractTest {
    ...
    @Test
    public void standardWidgetTargetsLauncher450RecursiveWidgetFrameOwner()
            throws Exception {
        ...
        assertTrue(source.contains("group.getChildCount() != 1"));
        assertTrue(source.contains("View content = group.getChildAt(0);"));
        assertTrue(source.contains("content.findViewById(android.R.id.widget_frame)"));
        assertTrue(source.contains("setBackground(null)"));

        assertFalse(source.contains("child.getTag(android.R.id.widget_frame)"));
