    ...
    static String apply(String source) {
        ...
        corrected = replaceExactlyOnce(
                ...
                GEOMETRY_ANCHOR
                        ...
                        + "        float e = 1.25;\n"
                        + "        float dL = prismalShapeDistAtRoot(rootPx - vec2(e, 0.0));\n"
                        + "        float dR = prismalShapeDistAtRoot(rootPx + vec2(e, 0.0));\n"
                        + "        float dT = prismalShapeDistAtRoot(rootPx - vec2(0.0, e));\n"
                        + "        float dB = prismalShapeDistAtRoot(rootPx + vec2(0.0, e));\n"
                        + "        float dTL = prismalShapeDistAtRoot(rootPx + vec2(-e, -e));\n"
                        + "        float dTR = prismalShapeDistAtRoot(rootPx + vec2(e, -e));\n"
                        + "        float dBL = prismalShapeDistAtRoot(rootPx + vec2(-e, e));\n"
                        + "        float dBR = prismalShapeDistAtRoot(rootPx + vec2(e, e));\n"
                        + "        distMask = d0;\n"
                        + "        sdKy = d0;\n"
                        + "        edgeDist = -d0;\n"
                        + "        crMask = 0.0;\n"
                        ...
                        + "        float hTL = getHeightFromDist(dTL, tw);\n"
                        + "        float hTR = getHeightFromDist(dTR, tw);\n"
                        + "        float hBL = getHeightFromDist(dBL, tw);\n"
                        + "        float hBR = getHeightFromDist(dBR, tw);\n"
                        + "        hSig = getHeightFromDist(d0, tw);\n"
                        + "        float hgx = (hTR + 2.0 * hR + hBR) - (hTL + 2.0 * hL + hBL);\n"
                        + "        float hgy = (hBL + 2.0 * hB + hBR) - (hTL + 2.0 * hT + hTR);\n"
                        + "        gradHSig = vec2(hgx, -hgy) * 0.125;\n"
